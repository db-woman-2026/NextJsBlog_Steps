...
게시글 목록과 상세 화면을 연결해 읽기 흐름을 만듭니다.

API Route를 목록 화면과 연결합니다.
...
깨진 링크를 남겨두면 다음 두 원인을 구분하기 어렵습니다.

- API가 잘못된 것인지
- 라우팅이 아직 구현되지 않은 것인지

목록 화면에서 `/detail/[id]` 링크를 만들 때 상세 화면도 함께 만듭니다.

`step-5`에서는 목록과 상세 화면을 연결해 게시글 읽기 흐름을 완성합니다.
...
같은 변경에서 `/detail/[id]` 페이지도 만들기 때문에 이 링크는 404로 깨지지 않아야 합니다.
...
`/detail/${post._id}` 링크를 만들었다면 `app/detail/[id]/page.js`도 같은 단계에서 만들어야 합니다. 그래야 목록을 클릭했을 때 404가 나지 않습니다.
