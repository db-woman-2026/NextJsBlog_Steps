...
`create-next-app`으로 만든 기본 프로젝트를 블로그의 첫 화면 구조로 바꿉니다.

데이터베이스, API, 게시글 작성 기능은 포함하지 않습니다. 다음 세 가지를 확인합니다.

- `app` 폴더 안의 파일 위치와 브라우저 주소의 연결을 확인한다.
- 모든 페이지에 공통으로 보이는 `Header`와 `Footer`를 만든다.
- `next/link`의 `Link` 컴포넌트로 페이지 사이를 이동한다.

라우트와 공통 layout의 결과를 분리해 확인할 수 있도록 화면 구조만 만듭니다.

## 완료 결과

완료 후 확인할 주소는 다음과 같습니다.
...
간단한 문구만 두고, 필요한 경우 저작권, GitHub 링크, 회사 정보 등을 추가합니다.
...
Post와 Contact에는 화면 구조만 두고 form과 API 요청은 포함하지 않습니다.
