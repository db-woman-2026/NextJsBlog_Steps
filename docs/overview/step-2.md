...
데이터베이스와 API는 변경하지 않고 화면 구성 도구만 적용합니다.

## 왜 simpledotcss를 사용하는가

CSS 변경이 많으면 컴포넌트와 라우팅 변경을 구분하기 어렵습니다.
...
디자인 변경은 최소화합니다. `simpledotcss`는 기본 HTML 태그에 읽기 좋은 스타일을 적용하는 작은 CSS 라이브러리입니다. 별도 컴포넌트 없이 `h1`, `p`, `form`, `button` 같은 태그를 그대로 사용합니다.
