...
서버 코드에서 MongoDB에 연결하고 게시글 조회·생성·수정 함수를 분리합니다. 화면과 API Route는 변경하지 않습니다.
...
여러 책임이 한 파일에 섞이지 않도록 서버 쪽 데이터 함수를 분리합니다.
...
```js
const uri = "mongodb://localhost:27017/next_blog_practice";
```
...
이 오류는 연결 문자열이 없을 때 즉시 실패하게 만듭니다. 조용히 넘어가면 원인과 떨어진 위치에서 오류가 발생할 수 있습니다.
...
`dbName`은 환경 변수 `MONGODB_DB`가 있으면 그 값을 사용하고, 없으면 `"next_blog_practice"`를 사용합니다. 실습 이름은 `next_blog_`로 시작하게 제한해 기존 데이터베이스를 잘못 연결하는 일을 줄입니다.
...
1. `npm.cmd install mongodb@^7.4.0`을 실행한다.
...
이 파일들은 화면에서 호출되지 않으므로 MongoDB가 실행 중이 아니어도 빌드는 통과해야 합니다.
...
여기서 빌드가 실패한다면 대부분 import 경로, 문법 오류, 패키지 설치 누락 문제입니다.
