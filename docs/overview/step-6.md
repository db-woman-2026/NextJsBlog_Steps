...
홈으로 이동한 뒤에는 작성한 글 제목을 클릭해 `/detail/[id]` 상세 화면에서 저장된 제목과 본문을 확인할 수 있습니다. 상세 화면은 `step-5`에서 이미 만들었기 때문에 목록 링크가 404로 깨지지 않아야 합니다.
...
form은 작성, 수정, Contact 화면에서 재사용하므로 전역 스타일에 둡니다.
