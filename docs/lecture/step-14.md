...
이 문서는 이전 단계 실습 결과에서 시작해 `step-14` 수준의 기능을 완성하는 실습 자료입니다.
...
- `"use client";`가 추가되므로 더 이상 서버에서 `listPosts`를 직접 호출하지 않고 `/api/post`를 fetch합니다.
