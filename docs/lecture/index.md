...
`step-23` 브랜치에서 사용할 수 있는 실습형 강의 자료입니다.
...
JavaScript, React, Next.js 구조와 용어가 아직 익숙하지 않다면 실습 전에 [기초 강의](../basic/index.md)를 순서대로 읽습니다. 기초 문서의 코드는 프로젝트에 적용하는 diff가 아니라, 실습에서 만날 문법을 짧은 예제로 익히기 위한 자료입니다.
...
| `step-22` | [lecture/step-22.md](./step-22.md) | 게시글 작성/수정 form과 Contact form에 같은 Tailwind 입력/버튼 패턴을 적용합니다. |
| `step-23` | [lecture/step-23.md](./step-23.md) | 삭제 버튼, 전역 404, 상세 404, Error 화면을 Tailwind로 정리해 기본 UI 전환을 마무리합니다. |

## 실습 방식

1. 첫 실습을 시작하기 전에 `npm.cmd ci`를 실행해 프로젝트 의존성을 설치합니다.
2. 현재 단계의 개요를 읽고 완성할 기능을 확인합니다.
3. 각 작업 단위의 설명을 읽은 뒤, `수정할 파일` 링크로 프로젝트 내부 파일을 엽니다.
4. `코드 변경` diff에서 `+` 줄은 추가하고 `-` 줄은 제거합니다.
5. npm 명령으로 자동 변경되는 파일은 입력하지 않고 결과만 확인합니다.
