...
npm install -D tailwindcss@4.3.2 @tailwindcss/postcss@4.3.2 postcss@8.5.22
```

위 명령은 `package.json`, `package-lock.json`, `node_modules`를 함께 갱신합니다. `Cannot find module '@tailwindcss/postcss'` 오류가 나오면 설치 명령이 성공했는지 확인한 뒤 `npm install`을 다시 실행합니다.
