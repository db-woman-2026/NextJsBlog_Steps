...
기본 블로그의 라우팅, API, CRUD 흐름을 통합 점검합니다.
...
이 작업에는 코드 diff가 없습니다. 위 설명과 확인 항목으로 현재 기능을 검증합니다.
...
삭제한 기본 asset이 import되지 않는지 lint와 build로 확인합니다. 결과와 확인 방법을 한 문장으로 기록합니다. 실험값은 검사를 마치면 원래대로 복구합니다.

## 마무리 확인

- 애플리케이션 코드는 변경하지 않습니다.
- 위 체크리스트의 화면 흐름이 실제로 이어지는지 확인합니다.
