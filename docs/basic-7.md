# Basic 7. 게시글 수정 화면 만들기

## 이 단계의 목표

`basic-7` 브랜치는 게시글 수정 화면을 추가하는 단계입니다.
...
3. 아직 상세 페이지가 없으므로, API 응답이나 MongoDB에서 게시글 `_id`를 확인합니다.
4. 브라우저에서 `/post/게시글id` 주소로 이동합니다.
5. 기존 제목과 본문이 form에 채워지는지 확인합니다.
6. 내용을 수정하고 저장합니다.
7. 홈 화면으로 이동하는지 확인합니다.
...
아직 게시글 상세 화면이 없습니다. 수정 화면 주소를 직접 입력하면 동작하지만, 사용자가 자연스럽게 상세 화면에서 수정 화면으로 이동하는 흐름은 다음 단계에서 완성합니다.
