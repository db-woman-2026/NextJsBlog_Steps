...
- 경로 예시는 `$HOME\dongbu\NextJsBlog_Steps`를 사용하고 공백 경로, `.env.local` 복사, UTF-8, 줄바꿈, 방화벽 안내를 확인합니다.
- 대괄호가 들어간 App Router 경로는 `-LiteralPath`로 처리합니다.
- `npm.ps1` 실행 정책 오류는 `npm.cmd`와 `npx.cmd`로 우회하고 실행 정책 변경을 필수 절차로 두지 않습니다.
- MongoDB는 Windows 서비스와 `next_blog_` 접두사의 전용 실습 DB만 사용합니다.
